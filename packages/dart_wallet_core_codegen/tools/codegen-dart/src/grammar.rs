...
fn parse_dir(path: &Path, mut main_dir: CHeaderDirectory) -> Result<CHeaderDirectory> {
    let entries = std::fs::read_dir(path).map_err(|e| Error::IoError(e))?;
    for entry in entries {
        let child_path = entry.map_err(|e| Error::IoError(e))?.path();
...
fn parse_file(path: &Path, mut main_dir: CHeaderDirectory) -> Result<CHeaderDirectory> {
    ...
    let parent = path.parent().ok_or(Error::io_error_other(format!(
        "File not found: {}",
        path.display()
    )))?;
    ...
    ($name:ident, $char:expr) => {
        ...
        impl ParseTree for $name {
            ...
            fn derive(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
                ...
                Err(Error::io_error_other(format!("Error",)))
            }
        }
    };
}
...
impl ParseTree for GEnumDecl {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        if string != "enum" {
            return Err(Error::io_error_other(format!("Error")));
        }
        ...
        loop {
            ...
            let number = if number.0.starts_with("0x") {
                usize::from_str_radix(&number.0[2..], 16)
                    .map_err(|_| Error::io_error_other(format!("Error parsing number")))?
            } else {
                number
                    .0
                    .parse::<usize>()
        ...
        if variants.is_empty() {
            return Err(Error::io_error_other(format!("Error variants are empty")));
        }
...
impl ParseTree for GEof {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        if slice.is_some() {
            return Err(Error::io_error_other(format!("Slice is not empty")));
        }
...
impl ParseTree for GPrimitive {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        let derived = match slice.as_str() {
            ...
            _ => {
                return Err(Error::io_error_other(format!("Error parsing primitive")));
            }
        };
...
impl ParseTree for GStructName {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        let (string, handle) = reader.read_until::<GSeparators>()?;
        if string != "struct" {
            return Err(Error::io_error_other(format!("Error parsing struct name")));
        }
...
impl ParseTree for GEnumName {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        let (string, handle) = reader.read_until::<GSeparators>()?;
        if string != "enum" {
            return Err(Error::io_error_other(format!("Error parsing enum name")));
        }
...
impl ParseTree for GMarker {
    ...
    fn derive<'a>(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        let der = match string.as_str() {
            ...
            _ => return Err(Error::io_error_other(format!("Error parsing marker"))),
        };
    ...
    fn derive<'a>(mut reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        loop {
            ...
            if let Ok(res) = T::derive(checked_out) {
                ...
                return Err(Error::io_error_other(format!("Error parsing continuum")));
            }
        }
    }
}
...
impl ParseTree for GNonAlphanumericItem {
    ...
    fn derive(mut reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        if GEof::derive(checked_out).is_ok() {
            return Err(Error::io_error_other(format!(
                "Error parsing non-alphanumeric item"
            )));
        } else {
            reader = pending.discard();
        }
    ...
    fn derive(mut p_reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        loop {
            ...
            if let Some(item) = item_der {
                ...
                return Err(Error::io_error_other(format!("Error parsing end of line")));
            }
...
impl ParseTree for GDefine {
    ...
    fn derive(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        if string != "#define" {
            return Err(Error::io_error_other(format!("Error parsing define")));
        }
...
impl ParseTree for GStaticVar {
    ...
    fn derive(reader: Reader) -> Result<DerivationResult<Self::Derivation>> {
        ...
        let (string, handle) = reader.read_until::<GSeparators>()?;
        if string != "static" {
            return Err(Error::io_error_other(format!("Error parsing static var")));
        }
...
impl ParseTree for GTypedef {
    ...
    fn derive(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        if string != "typedef" {
            return Err(Error::io_error_other(format!("Error parsing typedef")));
        }
...
impl ParseTree for GKeyword {
    ...
    fn derive(reader: Reader) -> Result<DerivationResult<Self::Derivation>> {
        ...
        if string.is_empty() {
            return Err(Error::io_error_other(format!("Error parsing keyword")));
        }
...
impl ParseTree for GCommentLine {
    ...
    fn derive(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        let (string, handle) = reader.read_until::<GSeparators>()?;
        if string != "//" && string != "///" {
            return Err(Error::io_error_other(format!("Error parsing comment line")));
        }
...
impl ParseTree for GTypeCategory {
    ...
    fn derive(reader: Reader<'_>) -> Result<DerivationResult<'_, Self::Derivation>> {
        ...
        Err(Error::io_error_other(format!(
